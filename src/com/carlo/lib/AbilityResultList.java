...
public class AbilityResultList {
	 ...
	 * @param day -1:前回の結果からインクリメントした値を入れる(前回がなければ占いは0,霊媒は1) それ以外ならそのまま入れる
	 * @param target 能力行使先のエージェント
	 * @param species 能力行使結果。GUARDEDでは使わない。
	 */
	public void addAbilityResult(Topic topic,int day,Agent agent,Agent target,Species species){
		if(day==-1 ){
		...
		resultList.add(new AbilityResult(topic,day,agent,target,species));
